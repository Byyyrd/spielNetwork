...
public class Client {
    ...
    public void sendMessage(Player player,Sword sword) {
        try {
            ...
            message = new Message(player.x, player.y,name, sword.rotation, slingshot.playerRotation,player.isSlingshotPickedUp(), slingshot.mousePos.getX(), slingshot.mousePos.getY(),isClicked, player.swordPickedUp, minePlaced, exploded,panel.message, panel.ui.normHp, panel.ui.death, player.swordDamage, player.slingshotDamage, player.mineDamage, player.mineTime, player.maxMines);
            if(!panel.serverDown) {
